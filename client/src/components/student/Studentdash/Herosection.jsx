...
const HeroSection = ({ user, onContinue }) => {
  return (
    <div
      className="p-10 rounded-xl shadow-lg flex flex-col md:flex-row items-center justify-between text-[var(--color-edgenius-button-text)] relative overflow-hidden"
      style={{
        background:
          "linear-gradient(to bottom right, var(--color-edgenius-accent-dark), var(--color-edgenius-accent-medium))",
      }}
    >
      ...
      <div className="relative z-10 text-center md:text-left md:w-2/3 mb-6 md:mb-0">
        ...
        <button
          onClick={onContinue}
          className="bg-[var(--color-edgenius-accent-dark)] text-[var(--color-edgenius-button-text)] px-8 py-4 rounded-full text-lg font-bold shadow-xl flex items-center justify-center group
                     hover:bg-[var(--color-edgenius-accent-dark)] hover:scale-105 transition-all duration-300 ease-out transform"
      ...
      <div className="relative z-10 md:w-1/3 flex flex-col justify-center items-center">
        {/* Profile Avatar */}
        <div className="mb-6 ml-42 relative">
          <div className="w-32 h-32 rounded-full border-4 border-[var(--color-edgenius-bg-lightest)] overflow-hidden shadow-2xl">
            {user.avatar ? (
              <img
                src={user.avatar}
              ...
              <div className="w-full h-full bg-[var(--color-edgenius-bg-lightest)] flex items-center justify-center text-[var(--color-edgenius-accent-dark)] text-4xl font-bold">
                {user.name.charAt(0).toUpperCase()}
              </div>
            )}
          </div>
          <div className="absolute -top-2 -right-2">
            <IoSparklesOutline className="text-[var(--color-edgenius-bg-lightest)] text-3xl opacity-70" />
          </div>
        </div>
      </div>
    </div>
